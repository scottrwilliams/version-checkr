...
class S3 {
  constructor(){
    this.getObject = () => ({
      promise: () => Promise.resolve({Body: 'cert'})
    });
  }
}
...
class GitHubApi {
  constructor(){
    this.authenticate = () => {};
    this.apps = {
      createInstallationToken: () => Promise.resolve({data: {token: "1"}})
    };
    this.repos = {
      ...
      createStatus: (status) => Promise.resolve({data: {description: status.description}})
    };
 }
}
...
describe( 'versioncheckr', () => {
  ...
  [
    {event: 'pull_request', action: 'create'},
    {event: 'pull_request', action: 'fork'},
    {event: 'release', action: 'published'},
    {event: 'organization', action: 'member_added'}
  ].forEach( (data) => {
  ...
  [
    ...
    {oldVersion: '0.0.0', newVersion: '1.0.0'},
    {oldVersion: '1.0.0', newVersion: '1.1.0'},
    {oldVersion: '9.9.99', newVersion: '10.0.0'}
  ].forEach( (data) => {
  ...
  [
    ...
    {oldVersion: '1.0.0', newVersion: '0.0.0'},
    {oldVersion: '1.0.1', newVersion: '1.0.0'},
    {oldVersion: '1.1.0', newVersion: '1.0.9'}
  ].forEach( (data) => {
